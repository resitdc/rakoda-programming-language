            ...
            func: Arc::new(|vm_ctx: &mut dyn VmContext, args: Vec<Value>| {
                ...
                unsafe {
                    match Library::new(&path) {
                        Ok(lib) => {
                            let idx = vm_ctx.get_heap_mut().alloc(HeapData::FfiLibrary(Arc::new(lib)));
        ...
        let definisi_obj = FungsiBawaanVM {
            ...
            func: Arc::new(|vm_ctx: &mut dyn VmContext, args: Vec<Value>| {
                ...
                let lib_arc = match &args[0] {
                    Value::Kamus(idx) => {
                        let id_val = if let HeapData::Kamus(k) = &vm_ctx.get_heap_mut().objects[*idx].data {
                            k.get("id").cloned()
                        } else {
                            None
                        };
                        match id_val {
                            Some(Value::Angka(n)) => {
                                if let HeapData::FfiLibrary(l) = &vm_ctx.get_heap_mut().objects[n as usize].data {
                                    l.clone()
                                } else {
                ...
                match &args[2] {
                    Value::Array(idx) => {
                        let arr_clone = if let HeapData::Array(arr) = &vm_ctx.get_heap_mut().objects[*idx].data {
                            arr.clone()
                        } else {
                        ...
                        for item in arr_clone {
                            if let Value::String(s_idx) = item {
                                if let HeapData::String(s) = &vm_ctx.get_heap_mut().objects[s_idx].data {
                ...
                let dynamic_call_obj = FungsiBawaanVM {
                    ...
                    func: Arc::new(move |call_ctx: &mut dyn VmContext, call_args: Vec<Value>| {
                        if call_args.len() != arg_types_str.len() {
                            return Err(format!("Fungsi {} mengharapkan {} argumen", nama_fungsi, arg_types_str.len()));
                        }

                        unsafe {
                            let sym: Symbol<unsafe extern "C" fn()> = match lib_arc.get(nama_fungsi.as_bytes()) {
                                Ok(s) => s,
                                Err(_) => return Err(format!("Simbol fungsi {} tidak ditemukan di library", nama_fungsi)),
                            };
                            ...
                            let cif = libffi::middle::Cif::new(ffi_arg_types.into_iter(), ffi_ret_type);
                                ...
                                match t.as_str() {
                                    "int" => {
                                        let v = match &call_args[i] {
                                            ...
                                            _ => return Err(format!("Argumen ke-{} harus angka (int)", i+1)),
                                        };
                                        int_vals.push((i, v));
                                    }
                                    "float" => {
                                        let v = match &call_args[i] {
                                            ...
                                            _ => return Err(format!("Argumen ke-{} harus angka (float)", i+1)),
                                        };
                                        float_vals.push((i, v));
                                    }
                                    "string" => {
                                        let s = match &call_args[i] {
                                            Value::String(idx) => {
                                                if let HeapData::String(s_val) = &call_ctx.get_heap_mut().objects[*idx].data {
                                                    s_val.clone()
                                                } else {
                                                    return Err(format!("Argumen ke-{} harus string", i+1));
                                                }
                                            }
                                            _ => return Err(format!("Argumen ke-{} harus string", i+1)),
                                        };
                                    ...
                                    "pointer" => {
                                        let v = match &call_args[i] {
                                            ...
                                            Value::Kosong => 0,
                                            _ => return Err(format!("Argumen pointer harus angka memori")),
                                        };
                            ...
                            for (i, t) in arg_types_str.iter().enumerate() {
                                match t.as_str() {
                                    ...
                                    "float" => ffi_args.push(libffi::middle::Arg::new(&vals_f32[i])),
                                    "string" => ffi_args.push(libffi::middle::Arg::new(&vals_ptr[i])),
                                    "pointer" => ffi_args.push(libffi::middle::Arg::new(&vals_ptr[i])),
                                    _ => unreachable!(),
                                }
                            }
                            ...
                            match ret_type_str.as_str() {
                                ...
                                "string" => {
                                    ...
                                    if res.is_null() {
                                        ...
                                        let s_idx = call_ctx.get_heap_mut().alloc(HeapData::String(c_str.to_string_lossy().into_owned()));
                ...
                let dynamic_call_idx = vm_ctx.get_heap_mut().alloc(HeapData::FungsiBawaan(dynamic_call_obj));
