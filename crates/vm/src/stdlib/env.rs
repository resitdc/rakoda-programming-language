//! VM module registration for env.
//! Delegates all logic to crates/stdlib/env.rs via adapter.
...
pub fn register(vm: &mut VM) {
    ...
    // Call dotenv() to load .env file automatically (existing behaviour).
    let _ = dotenvy::dotenv();

    for (nama, func) in stdlib::env::fungsi_env() {
