    ...
    env.borrow_mut().set(
        ...
        Objek::FungsiBawaan(|args| {
                ...
                && let (Objek::String(url), Objek::Kamus(config_map)) = (&args[0], &args[1])
            {
                ...
                let (_, http_config) = request::apply_config(ureq::get(url), &config_map.borrow());

                // Extract file path
                let file_path = if let Some(Objek::String(p)) = config_map.borrow().get("simpan") {
                    p.clone()
                } else {
                ...
                for attempt in 0..max_attempts {
                    ...
                    match result {
                        Ok(mut resp) => {
                            ...
                            respon_map
                                .insert("status_text".to_string(), Objek::String("OK".to_string()));
                            ...
                            respon_map.insert("body".to_string(), Objek::String("".to_string()));
                            respon_map.insert(
                                "header".to_string(),
