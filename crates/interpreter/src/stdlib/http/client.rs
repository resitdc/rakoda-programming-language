...
fn resolve_url(base_config: &HashMap<String, Objek>, url: &str) -> String {
    ...
    if let Some(Objek::String(base_url)) = base_config.get("base_url") {
                    ...
                    if !req_args.is_empty()
                        && let Objek::String(url) = &req_args[0] {
                            ...
                            let req_config = req_args
                                ...
                                .unwrap_or_default();
                            let merged_config =
                                request::merge_configs(&base_config_get, &req_config);
                            let final_url = resolve_url(&merged_config, url);

                            let (_, http_config) = request::apply_config(
                                ureq::get("http://localhost"),
                                &merged_config,
                            );
                            ...
                            for attempt in 0..=http_config.max_retries {
                                let (req, _) = request::apply_config(
                                    agent_get.get(&final_url),
                                    &merged_config,
                                );
                                match req.call() {
                                    Ok(res) => {
                                        return response::build_response_objek(
                                            res,
                                            start.elapsed(),
                                        );
                                    }
                    ...
                    if !req_args.is_empty()
                        && let Objek::String(url) = &req_args[0] {
                            ...
                            let req_config = req_args
                                ...
                                .unwrap_or_default();
                            let merged_config =
                                request::merge_configs(&base_config_del, &req_config);
                            let final_url = resolve_url(&merged_config, url);
                    ...
                    if !req_args.is_empty()
                        && let Objek::String(url) = &req_args[0] {
                            ...
                            let req_config = req_args
                                ...
                                .unwrap_or_default();
                            let merged_config =
                                request::merge_configs(&base_config_post, &req_config);
                            let final_url = resolve_url(&merged_config, url);
                    ...
                    if !req_args.is_empty()
                        && let Objek::String(url) = &req_args[0] {
                            ...
                            let req_config = req_args
                                ...
                                .unwrap_or_default();
                            let merged_config =
                                request::merge_configs(&base_config_put, &req_config);
                            let final_url = resolve_url(&merged_config, url);
                    ...
                    if !req_args.is_empty()
                        && let Objek::String(url) = &req_args[0] {
                            ...
                            let req_config = req_args
                                ...
                                .unwrap_or_default();
                            let merged_config =
                                request::merge_configs(&base_config_patch, &req_config);
                            let final_url = resolve_url(&merged_config, url);
