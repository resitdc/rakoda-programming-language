...
fn baca_impl(args: &[NilaiRpl]) -> Result<NilaiRpl, String> {
    ...
    match ext.as_str() {
        ...
        _ => Err(format!("Format dokumen '{}' belum didukung. Gunakan pdf, docx, xlsx, atau xls.", ext)),
    }
}
...
fn baca_docx(path: &Path) -> Result<NilaiRpl, String> {
    ...
    let mut archive = zip::ZipArchive::new(file).map_err(|e| format!("Gagal membuka archive DOCX: {}", e))?;
    
    let mut document_xml = archive.by_name("word/document.xml").map_err(|e| format!("Gagal menemukan word/document.xml: {}", e))?;
    let mut isi = String::new();
    std::io::Read::read_to_string(&mut document_xml, &mut isi).map_err(|e| format!("Gagal membaca XML DOCX: {}", e))?;
...
fn baca_excel(path: &Path) -> Result<NilaiRpl, String> {
    ...
    let mut workbook = open_workbook_auto(path).map_err(|e| format!("Gagal membuka Excel: {}", e))?;
