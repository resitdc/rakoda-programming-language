...
class CodeField extends StatefulWidget {
  ...
  /// The currently active line number (1-indexed). Used for active line highlight.
  final int? activeLine;

  /// The background color for the active line highlight in the gutter.
...
class _CodeFieldState extends State<CodeField> {
  ...
  Widget build(BuildContext context) {
    ...
    final editingField = Theme(
      ...
      child: LayoutBuilder(
        builder: (BuildContext context, BoxConstraints constraints) {
          if (widget.wrap) {
            // Word wrap mode: no horizontal scrolling, text wraps at container edge
            return widget.expands ? Expanded(child: codeField) : codeField;
          }
    ...
    final actualLineHeight = painter.preferredLineHeight;
    ...
    return FocusableActionDetector(
      ...
      child: Container(
        ...
        child: ClipRect(
          child: Stack(
            children: [
              // Active line highlight — spans full width (gutter + code)
              if (widget.activeLine != null && widget.activeLineColor != null)
                ListenableBuilder(
                  listenable: _codeScroll!,
                  builder: (context, child) {
                    final scrollOffset = _codeScroll?.hasClients == true ? _codeScroll!.offset : 0.0;
                    final top = 16.0 + ((widget.activeLine! - 1) * actualLineHeight) - scrollOffset;
                    return Positioned(
                      top: top,
                      left: -8, // compensate for container's left padding
                      right: 0,
                      height: actualLineHeight,
                      child: Container(color: widget.activeLineColor),
                    );
                  },
                ),
              // Gutter + Code Row
