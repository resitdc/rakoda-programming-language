...
class GutterWidget extends StatelessWidget {
  ...
  /// The currently active line number (1-indexed). If null, no line is highlighted.
  final int? activeLine;

  /// The background color for the active line highlight.
  ...
  Widget _buildOnChange(BuildContext context, Widget? child) {
    ...
    // Apply active line highlight decoration to the active row
    if (activeLine != null && activeLineColor != null) {
      final activeLineIndex = _lineIndexToTableRowIndex(activeLine! - 1);
      if (activeLineIndex != null && activeLineIndex < tableRows.length) {
        final oldRow = tableRows[activeLineIndex];
        tableRows[activeLineIndex] = TableRow(
          decoration: BoxDecoration(color: activeLineColor),
