...
const AndroidIcon = () => (
  <svg fill="currentColor" height="24px" width="24px" version="1.1" id="Layer_1" viewBox="0 0 299.679 299.679">
    <g id="XMLID_197_">
...
const Download = () => {
  ...
  return (
    <Layout title="Download" description="Unduh Rakoda Compiler dan RPL Studio">
      <main className={styles.pageContainer}>
        
        <div className={styles.pageHeader}>
          <Heading as="h1" className={styles.pageTitle}>Unduhan Resmi</Heading>
          <p className={styles.pageSubtitle}>
            Mulai membuat keajaiban dengan Bahasa Indonesia. Unduh compiler Rakoda dan IDE RPL Studio.
          </p>
        </div>
        ...
        <div className={styles.cardContainer}>
          ...
          <div className={styles.downloadCard}>
            <div className={styles.cardTitle}>
              <TerminalIcon /> Rakoda Compiler (CLI)
            </div>
            <p className={styles.cardDesc}>
              Kompiler berbasis baris perintah (CLI) untuk mengeksekusi kode program Rakoda secara lokal di mesin Anda.
            </p>

            <div className={styles.osList}>
              {loading ? (
                <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                  <div className={styles.loader}></div> <span className="text--sm">Mencari versi...</span>
                </div>
              ) : release ? (
                <>
                  <div className="text--sm" style={{ marginBottom: '0.5rem' }}>
                    <span className="badge badge--secondary">Versi {release.tag_name}</span>
                  </div>
                  {release.assets && release.assets.length > 0 ? (
                    release.assets.map((asset: any) => (
                      <a key={asset.id} href={asset.browser_download_url} className={styles.osButton}>
                        <div className={styles.osButtonIcon}><TerminalIcon /></div>
                        <div className={styles.osButtonText}>
                          <span className={styles.osName}>Unduh {asset.name}</span>
                          <span className={styles.osArch}>CLI Executable</span>
                        </div>
                      </a>
                    ))
                  ) : (
                    <a href={release.html_url} target="_blank" className="button button--secondary button--block">
                      Lihat Rilis di GitHub
                    </a>
                  )}
                </>
              ) : (
                <a href="https://github.com/resitdc/rakoda-programming-language" target="_blank" className="button button--secondary button--block">
                  Kunjungi GitHub
                </a>
              )}
            </div>
          ...
          <div className={styles.downloadCard}>
            <div className={styles.cardTitle}>
              ...
              RPL Studio (IDE Resmi)
            </div>
            <p className={styles.cardDesc}>
              Editor kode profesional dengan fitur Autocompletion, HTTP Workspace, dan integrasi Database bawaan.
            </p>

            <div className={styles.osList}>
              <a href="https://drive.google.com/drive/folders/1_URCMaQvbogZpIY7P8aTjMorYIXm7yec?usp=drive_link" target="_blank" rel="noopener noreferrer" className={styles.osButton}>
                <div className={styles.osButtonIcon}><WindowsIcon /></div>
                <div className={styles.osButtonText}>
                  <span className={styles.osName}>Unduh untuk Windows</span>
                  <span className={styles.osArch}>Portable (.exe / .zip)</span>
                </div>
              </a>

              <a href="https://drive.google.com/drive/folders/1Ud2goNnyGtUCuIuw8J1kYsTK734MtC1K?usp=drive_link" target="_blank" rel="noopener noreferrer" className={styles.osButton}>
                <div className={styles.osButtonIcon}><MacIcon /></div>
                <div className={styles.osButtonText}>
                  <span className={styles.osName}>Unduh untuk MacOS</span>
                  <span className={styles.osArch}>Installer (.dmg)</span>
                </div>
              </a>

              <a href="https://drive.google.com/drive/folders/1k-v-_2ZqUluQciA5KiQjLXyjhGFp1ni_?usp=sharing" target="_blank" rel="noopener noreferrer" className={styles.osButton}>
                <div className={styles.osButtonIcon}><LinuxIcon /></div>
                <div className={styles.osButtonText}>
                  <span className={styles.osName}>Unduh untuk Linux</span>
                  <span className={styles.osArch}>Executable (.AppImage / .deb)</span>
                </div>
              </a>

              <a href="https://drive.google.com/drive/folders/1zl72gl8DgT2PgVi41yDgGkLEd1ba8h8L?usp=drive_link" target="_blank" rel="noopener noreferrer" className={styles.osButton}>
                <div className={styles.osButtonIcon}><AndroidIcon /></div>
                <div className={styles.osButtonText}>
                  <span className={styles.osName}>Unduh untuk Android</span>
                  <span className={styles.osArch}>Aplikasi (.apk)</span>
                </div>
              </a>
            </div>
          </div>
